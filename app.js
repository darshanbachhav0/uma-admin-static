...
// Events + Users; Unsplash integration (store ONLY the external link in DB).
...
(function () {
  ...
  let currentImageFile = null;              // local file (if selected)
  let currentImageExternalUrl = null;       // external link (Unsplash etc.)
  const EVENTS_PATH = 'events';
  let eventsRef = null;

  // ===== Unsplash integration (link-only) =====
  ...
  // No-key variant: follow Source redirect and keep final URL
  async function getUnsplashLinkNoKey(query) {
  ...
  async function suggestImageFromUnsplash(auto = false) {
    ...
    const setBusy = (b) => { if (!auto) { btnSuggestImage.disabled = b; btnSuggestImage.textContent = b ? 'Buscando...' : 'Sugerir desde Unsplash'; } };
    ...
    try {
      ...
      if (UNSPLASH_KEY) {
        ...
        const p = await getUnsplashLinkNoKey(query);
        link = p.imageUrl;
      }

      currentImageExternalUrl = link;   // store ONLY the link in DB
      currentImageFile = null;          // ensure we don't try to upload a file
      currentSuggestedAttribution = credit;

      // Preview
      evPreview.src = link;
      evPreview.classList.remove('hidden');
      if (!auto) toast('Imagen sugerida lista (enlace externo).');
    } catch (e) {
      console.error(e);
  ...
  // Local file chosen -> clear any external URL
  evImage.addEventListener('change', (e) => {
    const file = e.target.files && e.target.files[0];
    currentImageFile = file || null;
    currentImageExternalUrl = null; // because we're going to upload a file if present
  ...
  function openEventDialog(ev) {
    ...
    evPreview.classList.add('hidden'); evPreview.removeAttribute('src');
    if (ev && ev.imageUrl) {
      evPreview.src = ev.imageUrl;
      evPreview.classList.remove('hidden');
      // Keep it as "current external URL" so saving without changes preserves it
      currentImageExternalUrl = ev.imageUrl;
    }
  ...
  async function saveEvent() {
    ...
    try {
      // Auto Unsplash if no image supplied at all
      if (AUTO_UNSPLASH_IF_NO_IMAGE && !currentImageFile && !currentImageExternalUrl && tags.length) {
      ...
      // 1) External link wins (Unsplash etc.)
      if (currentImageExternalUrl) {
        imageUrl = currentImageExternalUrl;

      // 2) Local file upload (optional path if admin picked a file)
        ...
        btnSaveEvent.textContent = 'Subiendo imagen… 0%';
        ...
        await new Promise((resolve, reject) => {
          task.on('state_changed',
            (snap) => {
              ...
              btnSaveEvent.textContent = `Subiendo imagen… ${pct}%`;
            },
            reject,
            resolve
          );
      ...
      // 3) Editing without changing image -> keep previous
      ...
      if (currentSuggestedAttribution) {
        payload.imageCredit = currentSuggestedAttribution; // optional attribution object
      }
  ...
  async function deleteEvent(id = currentEventId) {
    ...
    try {
      ...
      // Delete entire folder event_images/{id} (best effort)
      try {
